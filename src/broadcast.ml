...
(* Note that [Single_value foo] and [List_or_series [foo]] are not the same thing, in the
...
let merge t2 t1 =
  ...
  | Single_value v1, List_or_series { values = vs2; list_arg_names; index } ->
    List_or_series { values = List.map vs2 ~f:(fun v2 -> v1, v2); list_arg_names; index }
  | List_or_series { values = vs1; list_arg_names; index }, Single_value v2 ->
    List_or_series { values = List.map vs1 ~f:(fun v1 -> v1, v2); list_arg_names; index }
  | ( List_or_series { values = vs1; list_arg_names = list_arg_names1; index = i1 }
    ...
    let index =
      ...
      | None, None -> None
    in
    let values, list_arg_names = zip_exn (vs1, list_arg_names1) (vs2, list_arg_names2) in
    List_or_series { values; list_arg_names; index }
;;
